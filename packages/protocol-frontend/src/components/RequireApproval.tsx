...
export default function RequireApproval(props: RequireApprovalProps) {
  ...
  const {data: allowance, refetch} = useQuery(["token", "balanceOf", account], () => token().allowance(account??"", props.for), {
    enabled: canWrite,
  })
  ...
  return (
    <Box>
      <Stack direction={"row"}>
        {props.children}
        {!enoughAllowance && (
          <Button onClick={onSubmit} variant={"contained"}>Approve</Button>
        )}
      </Stack>
