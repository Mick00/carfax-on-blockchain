...
export default function Delegation(){
  ...
  const { data: identities, isLoading, isStale } = useQuery(["contributors.datafor", account], async () => {
    const hashes = await Promise.all(ownsId.map( id => contributors.tokenURI(id)))
...
function DelegateBox(props: {contributorId: BigNumberish}){
  ...
  const addDelegate = useWriteContract(({address, tokenId}:{ address: string, tokenId: BigNumberish }) => contributorDelegation().delegate(tokenId, address));
  ...
  return (
    <Stack direction={"row"} spacing={1} alignItems={"center"}>
      <ControlledTextField name={"address"} label={"address"} control={control} required rules={{isValidAddress}}/>
      <Button onClick={handleSubmit(onSubmit)} variant={"contained"}>Delegate</Button>
...
function UndelegateBox(props: {contributorId: BigNumberish}){
  ...
  const addDelegate = useWriteContract(({address, tokenId}:{ address: string, tokenId: BigNumberish }) => contributorDelegation().undelegate(tokenId, address));
  ...
  return (
    <Stack direction={"row"} spacing={1} alignItems={"center"}>
      <ControlledTextField name={"address"} label={"address"} control={control} required rules={{isValidAddress}}/>
      <Button onClick={handleSubmit(onSubmit)} variant={"contained"}>Undelegate</Button>
      <TransactionState transaction={addDelegate}/>
    </Stack>
  )
}
